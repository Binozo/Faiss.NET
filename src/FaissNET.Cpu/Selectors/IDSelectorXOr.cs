...
public sealed class IDSelectorXOr : IDSelector
{
    private readonly IDSelector _lhs;
    private readonly IDSelector _rhs;
    
    /// <summary>Symmetric difference of <paramref name="lhs"/> and <paramref name="rhs"/>.</summary>
    ...
    public IDSelectorXOr(IDSelector lhs, IDSelector rhs) : base(CreateHandle(lhs, rhs))
    {
        _lhs = lhs;
        _rhs = rhs;
    }
    ...
    private static IntPtr CreateHandle(IDSelector lhs, IDSelector rhs)
    {
        ...
        FaissErrorHandler.ThrowIfError(
            Native.faiss_IDSelectorXOr_new(out IntPtr ptr, lhs.SafeHandle, rhs.SafeHandle)
        );
